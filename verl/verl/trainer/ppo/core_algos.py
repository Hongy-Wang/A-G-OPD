...
@register_adv_est(AdvantageEstimator.RSOPD)
def compute_rsopd_outcome_advantage(old_log_probs: torch.Tensor, teacher_log_prob: torch.Tensor, response_mask: torch.Tensor, config=None, **kwargs):
    """Compute reverse sampled-token OPD advantages.

    For response tokens sampled from the student policy,

        A_t = log pi_teacher(y_t)
              - log pi_old(y_t)

    where the teacher distribution may be conditioned on either
    the standard student prefix or additional privileged information.

    Args:
        old_log_probs:
            Student log-probabilities on sampled response tokens.
            Shape: (batch_size, response_length).

        teacher_log_prob:
            Teacher log-probabilities on the same sampled response tokens.
            Shape: (batch_size, response_length).
    ...
    Returns:
        advantages:
            Token-level RSOPD advantages.

        returns:
            Compatibility placeholder for the current verl training
            and metric interface. RSOPD itself does not define
            value-function returns.
    """
    with torch.no_grad():
        advantages = teacher_log_prob - old_log_probs
        advantages = advantages * response_mask

        # RSOPD does not require value-function returns.
        # Keep this tensor only for compatibility with the current
        # trainer / metric interface.
